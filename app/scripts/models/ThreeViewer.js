define(['backbone', 'Viewport', 'staticHelpers', 'GeometryExport'], function (Backbone, Viewport, staticHelpers, GeometryExport) {
    ...
    function toThreeGeom(rawGeom) {
        ...
        for (; i < rawGeom.vertices.length; i++) {
            vertice = rawGeom.vertices[i];
            threeGeom.vertices.push(new THREE.Vector3(vertice[0], vertice[1], vertice[2]));
        }

        for (i = 0; i < rawGeom.faces.length; i++) {
            face = this.createFace(rawGeom.faces[i]);
    ...
    function addPoints(rawGeom, threeGeom) {
    ...
    function addLineStrip(rawGeom, threeGeom) {

        var line,
            i = 0;

        for (; i < rawGeom.linestrip.length; i++) {
            line = rawGeom.linestrip[i];
            threeGeom.vertices.push(new THREE.Vector3(line[0], line[1], line[2]));
        }
