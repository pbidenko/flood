define(['Node', 'FLOOD', 'staticHelpers'], function (Node, FLOOD, helpers) {
    ...
    return Node.extend({
        ...
        updateValue: function (values) {
            ...
            if (!this.get('extra').inputs || !helpers.arraysEqual(this.get('extra').inputs, codeBlock.InPorts)) {
                this.get('extra').inputs = codeBlock.InPorts;
                updated = true;
            }

            if (!this.get('extra').outputs || !helpers.arraysEqual(this.get('extra').outputs, codeBlock.OutPorts)) {
